...
set_credentials() {
    ...
    podman run --rm -ti registry.access.redhat.com/ubi10/httpd-24 htpasswd -Bbn developer $PASS_DEVELOPER > htpasswd.developer
    stop_if_failed $? "failed to set developer password"
    podman run --rm -ti registry.access.redhat.com/ubi10/httpd-24 htpasswd -Bbn kubeadmin $PASS_KUBEADMIN > htpasswd.kubeadmin
    stop_if_failed $? "failed to set kubeadmin password"
    podman run --rm -ti registry.access.redhat.com/ubi10/httpd-24 htpasswd -Bbn redhat $PASS_REDHAT > htpasswd.redhat
